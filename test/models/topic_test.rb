...
class TopicTest < ActiveSupport::TestCase
  test 'should validate the presence of topic name' do
    ...
    assert !Kinney::Topic.new(:description => 'Desc').valid?
  end

  test 'should validate uniqueness of name' do
    assert !Kinney::Topic.new(:name => 'Roots', :description => 'Desc').valid?
  end

  test 'should validate presence of description' do
    assert !Kinney::Topic.new(:name => 'Name').valid?
  end
  ...
  test "only topics with clips should be returned by default" do
    ...
    assert !Kinney::Topic.with_clips.include?(kinney_topics(:academics))
  end
